                                ...
                                <div class="col-span-12 p-2">
                                  <div class="rounded-2xl border border-indigo-100 bg-gradient-to-r from-indigo-50 to-white p-4 shadow-sm dark:border-gray-700 dark:from-gray-900 dark:to-gray-800">
                                    <div class="flex flex-wrap items-start justify-between gap-3">
                                      <div><div class="flex items-center gap-2"><p class="text-xs font-semibold uppercase tracking-wide text-indigo-600">Settlement wallet</p><a href="{{ route('dashboard') }}" class="grid h-7 w-7 place-items-center rounded-lg text-indigo-600 hover:bg-indigo-100" title="Refresh settlement balance" aria-label="Refresh settlement balance"><svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 11a8.1 8.1 0 0 0-15.5-2M4 4v5h5M4 13a8.1 8.1 0 0 0 15.5 2M20 20v-5h-5"/></svg></a></div><p class="mt-1 text-2xl font-bold text-gray-900 dark:text-white">₦{{ number_format((float)($settlement_wallet?->available_balance ?? 0), 2) }}</p><p class="text-xs text-gray-500">Reserved: ₦{{ number_format((float)($settlement_wallet?->reserved_balance ?? 0), 2) }}</p></div>
