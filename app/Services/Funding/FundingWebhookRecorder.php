...
{
    public function record(FundingProvider $provider, string $externalEventId, array $payload, ?AffiliateFundingProviderConfig $config = null): array
    {
        try {
            $event = FundingWebhookEvent::create([
                'funding_provider_id' => $provider->id,
                'parent_business_id' => $config?->parentFundingProvider?->parent_business_id,
