                  ...
                  <div class="mt-3">
                    <div id="pills-with-brand-color-2" class="" role="tabpanel" aria-labelledby="pills-with-brand-color-item-2">
                      <div class="overflow-auto">
                        ...
                        <div x-data="planTable()" class="p-4">
                          <!-- Filters -->
                          <div class="flex flex-wrap gap-3 mb-4">
                              <select x-model="filters.size" class="border rounded p-2">
                                  <option value="">All Sizes</option>
                                  <template x-for="size in uniqueSizes" :key="size">
                                      <option :value="size" x-text="size + ' MB'"></option>
                                  </template>
                              </select>
                      
                              <select x-model="filters.network" class="border rounded p-2">
                                  <option value="">All Networks</option>
                                  <template x-for="network in uniqueNetworks" :key="network.id">
                                      <option :value="network.id" x-text="network.name"></option>
                                  </template>
                              </select>
                      
                              <select x-model="filters.validity" class="border rounded p-2">
                                  <option value="">All Validities</option>
                                  <template x-for="days in uniqueValidities" :key="days">
                                      <option :value="days" x-text="days + ' Days'"></option>
                                  </template>
                              </select>
                      
                              <button @click="search()" class="px-4 py-2 bg-blue-600 text-white rounded">
                                  Search
                              </button>
                          </div>
                              ...
                              <table class="min-w-full border border-gray-200 dark:border-gray-700 rounded-lg">
                                  ...
                                  <tbody>
                                      <template x-for="plan in plans" :key="plan.id">
                                          <tr>
                                              <td class="px-4 py-2" x-text="plan.product_plan"></td>
                                              <td class="px-4 py-2" x-text="plan.data_size_in_mb"></td>
                                              <td class="px-4 py-2" x-text="plan.validity_in_days"></td>
                                              <td class="px-4 py-2" x-text="plan.network_id"></td>
                                              <td class="px-4 py-2" x-text="plan.automation"></td>
                                              <td class="px-4 py-2">
                                                  <span class="px-2 py-1 rounded text-xs"
                                                        :class="plan.visibility == 1 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'"
                                                        x-text="plan.visibility == 1 ? 'Yes' : 'No'">
                                                  </span>
                                              </td>
                                          </tr>
                                      </template>
...
function planTable() {
    return {
        ...
        uniqueSizes: [100, 200, 500, 1000, 2000, 3000, 5000, 10000], // or fetch dynamically
        uniqueNetworks: [
            {id: 1, name: 'MTN'},
            {id: 2, name: 'Airtel'},
            {id: 3, name: 'Glo'},
        ...
        uniqueValidities: [1, 3, 7, 14, 30, 60],

        async search() {
            let url = new URL("{{ route('plans.search') }}", window.location.origin);
        ...
        async init() {
            await this.search(); // load default plans
        }
    }
}
</script>
