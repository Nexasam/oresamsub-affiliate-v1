    ...
    <form method="POST" action="{{ $editingConnection ? route('parent-admin.provider-connections.update', $editingConnection['id']) : route('parent-admin.provider-connections.store') }}" x-data="providerConnectionForm(@js($initialForm), @js($adapters), @js($products), @js($runtimeFields), @js($credentialFields))" x-init="initialize()" class="space-y-5">
        ...
        <section x-show="selectedAdapter" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <template x-for="product in supportedProducts" :key="product.slug"><div x-show="activeProduct===product.slug" class="mt-5 space-y-5 rounded-2xl bg-slate-50 p-4">
                ...
                <div><div class="flex items-center justify-between"><h3 class="font-semibold">Request headers</h3><button type="button" @click="addHeader(product.slug)" class="rounded-lg border bg-white px-3 py-2 text-xs font-semibold">Add header</button></div><div class="mt-3 space-y-2"><template x-for="(row,index) in config(product.slug).request_headers" :key="row.id"><div class="grid gap-2 md:grid-cols-[1fr_140px_1fr_auto]"><input :name="`settings[product_configs][${product.slug}][request_headers][${index}][key]`" x-model="row.key" required placeholder="Authorization" class="rounded-lg border-slate-200"><select :name="`settings[product_configs][${product.slug}][request_headers][${index}][type]`" x-model="row.type" class="rounded-lg border-slate-200"><option value="credential">Credential</option><option value="runtime">Runtime</option><option value="literal">Literal</option></select><input :name="`settings[product_configs][${product.slug}][request_headers][${index}][value]`" x-model="row.value" class="rounded-lg border-slate-200"><button type="button" @click="config(product.slug).request_headers.splice(index,1)" class="text-sm font-semibold text-red-600">Remove</button></div></template></div></div>
...
document.addEventListener('alpine:init',()=>Alpine.data('providerConnectionForm',(initial,adapters,products,runtimeFields,credentialFields)=>({
    ...
    addHeader(slug){this.config(slug).request_headers.push({id:crypto.randomUUID(),key:'',type:'credential',value:this.allowedCredentialFields[0]||''})},
