...
<div class="space-y-5" x-data="productPlanWorkspace" @keydown.escape.window="closeOverlays()">
    ...
    <div x-cloak x-show="switchModalOpen" class="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/60 p-4" @click.self="closeSwitchModal()">
        <section class="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl" role="dialog" aria-modal="true" aria-label="Switch selected product plan connections">
            ...
            <form method="POST" action="{{ route('parent-admin.product-plans.routes.bulk-switch') }}" class="flex min-h-0 flex-1 flex-col" @submit="if (!confirm(`Apply the reviewed connection routes to ${switchRows.length} selected plans?`)) $event.preventDefault()">@csrf @method('PATCH')
                <div class="flex-1 overflow-y-auto p-5">
                    <label class="block text-sm font-semibold">Apply one connection to every row <span class="font-normal text-slate-400">(optional shortcut)</span><select x-model="targetConnectionId" @change="applyConnectionToAll()" class="mt-2 w-full rounded-xl border-slate-200"><option value="">Choose connection</option>@foreach($connections as $connection)<option value="{{ $connection->id }}">{{ $connection->name }} · {{ $connection->providerConnection?->name }}</option>@endforeach</select></label>
                    <div class="mt-5 overflow-x-auto rounded-xl border"><table class="w-full min-w-[850px] text-left text-sm"><thead class="bg-slate-50 text-xs uppercase text-slate-500"><tr><th class="p-3">Plan</th><th class="p-3">Current connection</th><th class="p-3">New connection</th><th class="p-3">New provider plan ID</th><th class="p-3">Status</th></tr></thead><tbody class="divide-y"><template x-for="(row,index) in switchRows" :key="row.id"><tr><td class="p-3"><input type="hidden" :name="`plans[${index}][product_plan_id]`" :value="row.id"><strong class="block text-xs" x-text="row.product_plan_name"></strong><span class="text-[11px] text-slate-400" x-text="row.category_name"></span></td><td class="p-3 text-xs" x-text="row.current_connection || 'No route'"></td><td class="p-3"><select :name="`plans[${index}][parent_provider_connection_id]`" x-model="row.target_connection_id" @change="prefillRow(row)" required class="w-full rounded-lg border-slate-200 text-sm"><option value="">Choose connection</option><template x-for="connection in connections" :key="connection.id"><option :value="String(connection.id)" x-text="connection.label"></option></template></select></td><td class="p-3"><input :name="`plans[${index}][provider_plan_id]`" x-model="row.provider_plan_id" required class="w-full rounded-lg border-slate-200 text-sm" placeholder="Required provider ID"></td><td class="p-3"><span class="rounded-full px-2 py-1 text-[10px] font-semibold" :class="row.target_connection_id && row.provider_plan_id ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'" x-text="row.target_connection_id && row.provider_plan_id ? (row.remembered ? 'Remembered' : 'Ready') : (!row.target_connection_id ? 'Connection required' : 'ID required')"></span></td></tr></template></tbody></table></div>
                </div>
                <div class="flex items-center justify-between border-t bg-slate-50 p-4"><p class="text-xs text-slate-500" x-text="`${switchRows.length} plans selected`"></p><button :disabled="switchRows.some(row => !row.target_connection_id || !row.provider_plan_id)" class="rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-40">Confirm connection switch</button></div>
            </form>
        </section>
    </div>
</div>
    ...
    Alpine.data('productPlanWorkspace',()=>({
        ...
        handleBulkSubmit(event){
            if(this.bulkAction==='switch_connection'){
                ...
                this.switchRows=this.selectedIds.map(id=>{const plan=JSON.parse(JSON.stringify(this.plans[String(id)])); const current=plan.known_routes.find(route=>route.current); return {...plan,current_connection:current?.connection_name || '',target_connection_id:'',provider_plan_id:'',remembered:false} });
        ...
        prefillRow(row){ const remembered=row.known_routes.find(route=>String(route.connection_id)===String(row.target_connection_id)); row.provider_plan_id=remembered?.provider_plan_id || ''; row.remembered=Boolean(remembered) },
        applyConnectionToAll(){ if(!this.targetConnectionId) return; this.switchRows.forEach(row=>{row.target_connection_id=String(this.targetConnectionId); this.prefillRow(row)}) }
    }));
