@extends('layouts.app')
@section('content')
<div class="main-content workspace-page"><div class="mx-auto max-w-5xl space-y-5">
    <x-workspace.page-header title="Settlement funding" description="Fund your business settlement wallet through virtual accounts issued with your parent’s approved funding providers." />
    @if(session('success'))<x-workspace.alert type="success">{{ session('success') }}</x-workspace.alert>@endif
    @if(session('error'))<x-workspace.alert type="error">{{ session('error') }}</x-workspace.alert>@endif
    ...
    <section class="workspace-panel p-5">
        ...
        <div class="mt-4 flex flex-wrap gap-2">@foreach($providers as $provider)<form method="POST" action="{{ route('admin.settlement-funding.generate',$provider) }}">@csrf<button class="workspace-btn-primary">Generate with {{ $provider->fundingProvider->name }}</button></form>@endforeach</div>
    </section>
